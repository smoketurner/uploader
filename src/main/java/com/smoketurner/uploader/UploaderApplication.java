...
public class UploaderApplication extends Application<UploaderConfiguration> {
    ...
    public void run(@Nonnull final UploaderConfiguration configuration,
        ...
        final EventLoopGroupConfiguration eventLoopConfig = EventLoopGroupConfiguration
                .builder().eventLoopGroup(workerGroup).build();
        final NettySdkHttpClientFactory nettyFactory = NettySdkHttpClientFactory
                .builder().eventLoopGroupConfiguration(eventLoopConfig).build();
        final ClientAsyncHttpConfiguration httpConfig = ClientAsyncHttpConfiguration
                .builder().httpClientFactory(nettyFactory).build();
        ...
        final S3AsyncClient s3 = S3AsyncClient.builder()
                ...
                .asyncHttpConfiguration(httpConfig).build();
        environment.lifecycle().manage(new AutoCloseableManager(s3));

        final Uploader uploader = new Uploader(s3, awsConfig);

        final UploadInitializer initializer = new UploadInitializer(nettyConfig,
        ...
        final ChannelFuture future = bootstrap.group(bossGroup, workerGroup)
                ...
                .childHandler(initializer).bind(nettyConfig.getListenPort());
